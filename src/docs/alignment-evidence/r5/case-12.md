# Case 12 alignment evidence

- Generated: 2026-09-03T20:22:41.679430+08:00
...
- Legacy raw: `raw/case-12.legacy.json`
- New raw: `raw/case-12.new.json`

## Legacy request and raw response

See `raw/case-12.legacy.json`. The `body` field contains the complete unmodified JSON or SSE stream; `meta` contains the full request and start/finish timestamps.

## New request and raw response

See `raw/case-12.new.json`. The `body` field contains the complete MCP SSE envelope; `parsed_tool_result` contains the complete parsed tool JSON payload.

## Field-level semantic comparison

Neither silently blocks. Legacy allows adds and warns in prose that ¥1280 exceeds budget; new add succeeds and emits BUDGET_EXCEEDED_WARNING. Warning semantics match.
