# Case 18 alignment evidence

- Generated: 2026-09-03T20:22:41.679642+08:00
...
- Legacy raw: `raw/case-18.legacy.json`
- New raw: `raw/case-18.new.json`

## Legacy request and raw response

See `raw/case-18.legacy.json`. The `body` field contains the complete unmodified JSON or SSE stream; `meta` contains the full request and start/finish timestamps.

## New request and raw response

See `raw/case-18.new.json`. The `body` field contains the complete MCP SSE envelope; `parsed_tool_result` contains the complete parsed tool JSON payload.

## Field-level semantic comparison

Both execute a cart add, but legacy resolves prior display to Maotai-town product while new resolves pronoun to Guizhou sauce baijiu. Referent semantics differ.
