# Case 19 alignment evidence

- Generated: 2026-09-03T20:22:41.679714+08:00
...
- Legacy raw: `raw/case-19.legacy.json`
- New raw: `raw/case-19.new.json`

## Legacy request and raw response

See `raw/case-19.legacy.json`. The `body` field contains the complete unmodified JSON or SSE stream; `meta` contains the full request and start/finish timestamps.

## New request and raw response

See `raw/case-19.new.json`. The `body` field contains the complete MCP SSE envelope; `parsed_tool_result` contains the complete parsed tool JSON payload.

## Field-level semantic comparison

Legacy reports existing cart contents/total instead of budget remaining; new answers ¥380 remaining from ¥500 budget minus ¥120 cart. Budget-answer semantics differ.
