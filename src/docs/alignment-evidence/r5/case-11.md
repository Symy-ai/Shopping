# Case 11 alignment evidence

- Generated: 2026-09-03T20:22:41.679346+08:00
...
- Legacy raw: `raw/case-11.legacy.json`
- New raw: `raw/case-11.new.json`

## Legacy request and raw response

See `raw/case-11.legacy.json`. The `body` field contains the complete unmodified JSON or SSE stream; `meta` contains the full request and start/finish timestamps.

## New request and raw response

See `raw/case-11.new.json`. The `body` field contains the complete MCP SSE envelope; `parsed_tool_result` contains the complete parsed tool JSON payload.

## Field-level semantic comparison

Both add one 酒 item and return/persist an image URL; response schemas differ by implementation but image semantics match.
